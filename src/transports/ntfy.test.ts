...
describe('ntfy transport specifics', () => {
  it('skips foreign (undecryptable) traffic on the topic', async () => {
    ...
    await t.send({ from: 'a', to: '*', kind: 'message', text: 'ours' })
  ...
  it('a different key sees nothing (E2E for real)', async () => {
    ...
    const t = party.connectAs()
    await t.join('a')
    await t.send({ from: 'a', to: '*', kind: 'message', text: 'secret' })
    const parsed = parseRef(party.ref)
    if (parsed.scheme !== 'ntfy') throw new Error('expected an ntfy ref')
  ...
  it('uses its own <ts>.<id> cursors (relay-independent)', async () => {
    ...
    const t = party.connectAs()
    await t.join('a')
    const sent = await t.send({ from: 'a', to: '*', kind: 'message', text: 'hello' })
  ...
  it('an anchor missing from the stream falls back to the cursor timestamp', async () => {
    ...
    const t = party.connectAs()
    await t.join('a')
    await t.send({ from: 'a', to: '*', kind: 'message', text: 'old news' })
  ...
  it('chunks long texts transparently and reassembles them on read', async () => {
    ...
    const sent = await sender.send({ from: 'a', to: '*', kind: 'message', text: longText })
  ...
  it('rejects texts above the total chunked limit', async () => {
    ...
    const t = party.connectAs()
    await t.join('a')
    await expect(t.send({ from: 'a', to: '*', kind: 'message', text: 'x'.repeat(100_000) })).rejects.toThrow(
      'too large',
    )
    await t.close()
  })
  ...
  it('retries politely on 429 and succeeds', async () => {
    ...
    const sent = await t.send({ from: 'a', to: '*', kind: 'message', text: 'made it' })
  ...
  it('a close by one instance is enforced on another long-lived instance', async () => {
    ...
    await veteran.send({ from: 'veteran', to: '*', kind: 'message', text: 'warm cache' })
    const closer = party.connectAs()
    await closer.join('closer')
    await closer.send({ from: 'closer', to: '*', kind: 'close', text: 'party closed by closer' })
    await expect(veteran.send({ from: 'veteran', to: '*', kind: 'message', text: 'too late' })).rejects.toThrow(
      'closed',
    )
    await veteran.close()
    await closer.close()
  ...
  it('an unparseable cursor falls back to the full stream', async () => {
    ...
    const t = party.connectAs()
    await t.join('a')
    await t.send({ from: 'a', to: '*', kind: 'message', text: 'still here' })
